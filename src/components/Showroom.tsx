...
function TrustStrip({ lang }: { lang: "en" | "zh" }) {
  const t = T[lang];
  return (
    <div className="bg-ink-strip text-paper px-[clamp(18px,4vw,56px)] py-2.5 flex items-center justify-center gap-x-[22px] gap-y-3 flex-wrap font-mono text-[10px] tracking-[0.2em] uppercase">
    ...
    <div>
      ...
      <div className="grid border-b border-ink [grid-template-columns:repeat(auto-fit,minmax(min(100%,400px),1fr))]">
        <div className="px-[clamp(20px,4vw,56px)] pt-[clamp(40px,6vw,96px)] pb-[clamp(36px,5vw,72px)] flex flex-col justify-between bg-paper min-w-0">
          <div>
            <div className="font-mono text-[10.5px] tracking-[0.24em] uppercase text-indigo font-medium mb-[26px]">
              {t.heroEyebrow}
            </div>
            <h1 className="font-serif font-normal m-0 text-[clamp(42px,7.4vw,88px)] leading-[0.92] tracking-[-0.02em] max-w-[13ch] [text-wrap:balance] keep-all">
              {t.heroTitle}
            </h1>
            ...
            <div className="flex flex-wrap gap-2.5 mt-[30px] pt-[22px] border-t border-line">
              {t.heroChips.map((c) => (
                <span
                  key={c}
                  className="border border-line rounded px-3 py-2 font-mono text-[10.5px] tracking-[0.1em] uppercase text-txt-2 keep-all"
                >
                  {c}
                </span>
              ))}
          ...
          <div className="flex gap-3 mt-10 flex-wrap">
            <Link
              href="/customiser"
              className="bg-ink text-paper px-[26px] py-4 font-sans text-[14px] font-medium tracking-[0.02em] rounded flex-1 min-w-[200px] text-center hover:bg-indigo hover:text-paper transition-colors"
            >
              {t.heroCta}
            </Link>
            <a
              href="#contact"
              className="bg-transparent text-ink border border-ink px-[26px] py-4 font-sans text-[14px] font-medium rounded inline-flex items-center justify-center flex-1 min-w-[160px] hover:bg-ink hover:text-paper transition-colors"
            >
              {t.heroCta2}
            </a>
          </div>
        ...
        <div className="relative h-[clamp(320px,46vw,620px)] min-w-0 overflow-hidden border-t border-ink">
          ...
          />
          <div className="absolute bottom-0 left-0 right-0 px-6 pt-10 pb-[18px] font-mono text-[10.5px] font-medium tracking-[0.18em] text-paper bg-[linear-gradient(to_top,rgba(11,13,18,0.85),rgba(11,13,18,0.5)_45%,transparent)] pointer-events-none whitespace-nowrap overflow-hidden text-ellipsis">
            {t.heroBadge}
          </div>
        </div>
      </div>
          ...
          >
            ...
            <div className="pt-4 px-0.5 flex flex-col gap-1.5">
              <div className="flex flex-col gap-0.5">
                <div className="font-serif font-normal text-[clamp(17px,1.5vw,20px)] tracking-[-0.005em] leading-[1.2] keep-all">
                  {zh ? p.cn : p.name}
                </div>
                {zh && (
